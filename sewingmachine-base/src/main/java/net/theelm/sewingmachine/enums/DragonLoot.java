...
import net.theelm.sewingmachine.objects.rewards.WeightedRewardEnchantedBook;
import net.theelm.sewingmachine.objects.rewards.WeightedRewardGenerator;
import net.theelm.sewingmachine.objects.rewards.WeightedRewardItem;
...
public final class DragonLoot {
    ...
    private static void initRewards() {
        ...
        if (SewConfig.get(SewCoreConfig.DRAGON_LOOT_END_ITEMS)) {
            ...
            DragonLoot.itemReward(2300, Items.WITHER_SKELETON_SKULL, (player, stack) -> stack.setCount(IntUtils.random(player.getRandom(), 3, 6)));
            DragonLoot.itemReward(1800, Items.ENCHANTED_GOLDEN_APPLE);
            DragonLoot.itemReward(350, Items.NETHERITE_SCRAP, (player, stack) -> stack.setCount(IntUtils.random(player.getRandom(), 1, 8)));
            DragonLoot.itemReward(2000, Items.SHULKER_SHELL, (player, stack) -> stack.setCount(IntUtils.random(player.getRandom(), 1, 3)));
